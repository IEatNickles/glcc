...
glcc_bool glcc_make_current(glcc_Context context);
glcc_bool glcc_swap_interval(int interval);
glcc_bool glcc_swap_buffers(glcc_Context context);
...
typedef glcc_bool(*GLCC_MAKECURRENTPROC)(glcc_Context);
typedef glcc_bool(*GLCC_SWAPINTERVALPROC)(int /* interval */);
typedef glcc_bool(*GLCC_SWAPBUFFERSPROC)(glcc_Context);
...
typedef EGLBoolean(*PFN_eglSwapInterval)(EGLDisplay, int);
...
struct {
  ...
  GLCC_SWAPINTERVALPROC swap_interval;
...
#define eglSwapInterval _glcc.egl.SwapInterval
...
glcc_bool glcc_make_current(glcc_Context ctx) {
  return _glcc.make_current(ctx);
}

glcc_bool glcc_swap_interval(int interval) {
  return _glcc.swap_interval(interval);
}

glcc_bool glcc_swap_buffers(glcc_Context ctx) {
  return _glcc.swap_buffers(ctx);
}
...
#if GLCC_BUILD_X11
glcc_bool _glcc_make_current_x11(glcc_Context ctx) {
  _glcc_Context *c = (_glcc_Context*)ctx;
  return eglMakeCurrent(_glcc.egl.display, c->egl.surface, c->egl.surface, c->egl.context);
}

glcc_bool _glcc_swap_interval_x11(int interval) {
  return eglSwapInterval(_glcc.egl.display, interval);
}

glcc_bool _glcc_swap_buffers_x11(glcc_Context ctx) {
  _glcc_Context *c = (_glcc_Context*)ctx;
  return eglSwapBuffers(_glcc.egl.display, c->egl.surface);
}
...
glcc_bool _glcc_init_x11(void *display) {
  ...
  _glcc.swap_interval = _glcc_swap_interval_x11;
...
glcc_bool _glcc_init_egl() {
  ...
  _glcc.egl.SwapInterval = _glcc_get_symbol(_glcc.egl.handle, "eglSwapInterval");
  if (!_glcc.egl.SwapInterval) return GLCC_FALSE;
